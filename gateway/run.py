...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="R1",
            location="gateway/run.py:GatewayRunner.__init__",
            message="init start",
            data={},
        )
        # endregion
        ...
        try:
            from extensions.hermes_spare24.bootstrap import load_extension
            from extensions.hermes_spare24.registry import GLOBAL_REGISTRY
            self._spare24_snapshot = load_extension(GLOBAL_REGISTRY)
        except ImportError:
            self._spare24_snapshot = None
        # region agent log
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="R1",
            location="gateway/run.py:GatewayRunner.__init__",
            message="init extension snapshot resolved",
            data={
                "has_snapshot": bool(self._spare24_snapshot),
                "platform_count": len(getattr(self._spare24_snapshot, "platform_factories", {}) or {}),
            },
        )
        # endregion
    ...
    async def start(self) -> bool:
        ...
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="R2",
            location="gateway/run.py:GatewayRunner.start",
            message="start entered",
            data={"platforms_configured": [p.value for p, c in self.config.platforms.items() if c.enabled]},
        )
        # endregion
        ...
        for platform, platform_config in self.config.platforms.items():
            ...
            _debug_log(
                ...
                hypothesis_id="R3",
                location="gateway/run.py:GatewayRunner.start",
                message="platform connect attempt",
    ...
    def _create_adapter(
        ...
        # Hook 2: extension platform factories — primary path
        # EMERGENCY ROLLBACK: Platform.MAX and Platform.BITRIX_EMAIL branches below are fallback only
        # Do not add business logic here
        if self._spare24_snapshot is not None:
            factory = self._spare24_snapshot.platform_factories.get(platform.value)
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="H1",
            location="gateway/run.py:_is_user_authorized",
            message="platform auth mapping resolved",
            data={
                "platform": source.platform.value if source.platform else "",
                "platform_allow_var": platform_env_map.get(source.platform, ""),
                "platform_allow_all_var": platform_allow_all_var,
                "user_id": user_id,
            },
        )
        # endregion
        ...
        _debug_log(
            ...
            hypothesis_id="H2",
            location="gateway/run.py:_is_user_authorized",
            message="allowlist values loaded",
            data={
                "platform": source.platform.value if source.platform else "",
                "platform_allowlist_present": bool(platform_allowlist),
                "global_allowlist_present": bool(global_allowlist),
                "platform_allowlist_preview": platform_allowlist[:64],
            },
        )
        # endregion
        ...
        if not platform_allowlist and not global_allowlist:
            ...
            _debug_log(
                ...
                hypothesis_id="H3",
                location="gateway/run.py:_is_user_authorized",
                message="no allowlist path taken",
                data={
                    "platform": source.platform.value if source.platform else "",
                    "gateway_allow_all": os.getenv("GATEWAY_ALLOW_ALL_USERS", "").lower(),
                },
            )
            # endregion
        ...
        authorized = bool(check_ids & allowed_ids)
        # region agent log
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="H4",
            location="gateway/run.py:_is_user_authorized",
            message="allowlist intersection result",
            data={
                "platform": source.platform.value if source.platform else "",
                "authorized": authorized,
                "check_ids": sorted(list(check_ids))[:5],
                "allowed_ids": sorted(list(allowed_ids))[:20],
            },
        )
        # endregion
        return authorized
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        _debug_log(
            run_id="pre-fix",
            hypothesis_id="H13",
            location="gateway/run.py:_handle_message",
            message="gateway handle_message entered",
            data={
                "platform": source.platform.value if source.platform else "",
                "user_id": source.user_id,
        ...
        if getattr(event, "internal", False):
            ...
            _debug_log(
                ...
                hypothesis_id="H5",
                location="gateway/run.py:_handle_message",
                message="unauthorized branch entered",
                data={
                    "platform": source.platform.value if source.platform else "",
                    "user_id": source.user_id,
                    "chat_type": source.chat_type,
                    "behavior": self._get_unauthorized_dm_behavior(source.platform),
                },
            )
            # endregion
